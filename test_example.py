...
class TestExample(unittest.TestCase):
    ...
    def test_eg(self):
        ...
        def kinetics_experiment(mip_option, 
            ...
            res, fim = calculator.extract_store_sol(start_budget, file_store_name)
            sol_list.append(res)
            obj_list.append(fim)
            ...
            for b in budget_ranges[1:]:
                ...
                res, fim = calculator.extract_store_sol(b, file_store_name)
                sol_list.append(res)
                obj_list.append(fim)

            return sol_list, obj_list
        
        ### Test A-optimality, LP

        ## The most often used options are listed as arguments here 
        mip_option_value = False # mixed integer problem or not
        objective_value = ObjectiveLib.A # objective, ObjectiveLib.A or ObjectiveLib.D 
        ...
        self.assertAlmostEqual(np.linalg.det(obj_set[1]), 5.3288, places=1)

        
        ### Test A-optimality, MILP
        ...
        self.assertAlmostEqual(np.linalg.det(obj_set[1]), 3.539, places=1)
        
        
        ### Test D-optimality, NLP
        ...
        # lp_A: iniitalize with lp_A solution
        # nlp_D: initialize with nlp_D solution
        initializer_option_value = "milp_A"
        # if run all results or just sensitivity test 
        rerun_all_paper_results_value = False
        ...
        self.assertAlmostEqual(np.linalg.det(obj_set[1]), 21.5379, places=1)
        
        ### Test D-optimality, MINLP
        ...
        self.assertAlmostEqual(np.linalg.det(obj_set[1]), 18.2956, places=1)
