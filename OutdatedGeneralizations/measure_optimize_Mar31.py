...
class MeasurementOptimizer:
    ...
    def continuous_optimization(self, cost_list, mixed_integer=False, obj="A", 
        ...
        if not fix: 
            ...
            if each_manual_number is not None:
            ...
            if discretize_time is not None:
    ...
    def continuous_optimization_reduced_form(self, cost_list, mixed_integer=False, obj="A", 
                                fix=False, sparse=False,
                                num_dynamic_t_name = None, 
                                discretize_time = None,
                                manual_number=20, budget=100, 
        ...
        """Continuous optimization problem formulation. 
        This reduced form is specifically for cases where there are no covariances, so that 
        the correlation decision variables can be deprecated. 
        ...
        # decision variables
        if mixed_integer:
            m.cov_y = pyo.Var(m.NumRes, initialize=initialize, bounds=(0,1), within=pyo.Binary)
        else:
            m.cov_y = pyo.Var(m.NumRes, initialize=initialize, bounds=(0,1), within=pyo.NonNegativeReals)
        ...
        def eval_fim(m, a, b):
            ...
            """
            if a <= b: 
                summi = sum(m.cov_y[i]*self.fim_collection[i*self.num_measure_dynamic_flatten+i][a][b] for i in range(self.num_measure_dynamic_flatten))
                return m.TotalFIM[a,b] == summi
            else:
                return m.TotalFIM[a,b] == m.TotalFIM[b,a]        
    
        def total_dynamic(m):
            return m.TotalDynamic==sum(m.cov_y[i] for i in range(self.num_static, self.num_measure_dynamic_flatten))
        
        ### cost constraints
        def cost_compute(m):
            return m.cost == sum(m.cov_y[i]*cost_list[i] for i in m.NumRes)+sum(m.if_install_dynamic[j]*dynamic_install_cost[j-self.num_static] for j in m.DimDynamic)
